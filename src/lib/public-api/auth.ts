...
export async function authenticatePublicApi(request: Request, required: ApiScope): Promise<PublicApiContext | Response> {
  ...
  if (!match || match[1].length > 256) return publicApiJson({ error: { code: 'UNAUTHORIZED', message: 'Invalid API key.' } }, 401)
  ...
  if (!row || !row.userId) return publicApiJson({ error: { code: 'UNAUTHORIZED', message: 'Invalid API key.' } }, 401)
  const user = await systemPrisma.user.findFirst({
    where: { id: row.userId, organizationId: row.organizationId, isActive: true },
    select: { id: true },
  })
  if (!user) return publicApiJson({ error: { code: 'UNAUTHORIZED', message: 'API key owner is no longer active.' } }, 401)
  const scopes = new Set(Array.isArray(row.scopes) ? row.scopes.filter((scope): scope is string => typeof scope === 'string') : [])
  if (!scopes.has(required)) return publicApiJson({ error: { code: 'FORBIDDEN', message: `API key requires ${required}.` } }, 403)
  const limited = await rateLimit(`public-api:${row.id}`, { limit: 600, windowMs: 60_000, failureMode: 'closed' })
  if (!limited.ok) return publicApiJson({ error: { code: 'RATE_LIMITED', message: 'Rate limit exceeded.' } }, 429)
