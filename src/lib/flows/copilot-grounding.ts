...
const graphRules =
  ...
  'Allowed node types: agent, tool, http, transform, filter, condition, switch, loop, parallel, stop, variable, data, code, humanReview, wait, note, output, join, ai, subflow, knowledge. ' +
  ...
  'Wait data: {mode, amount, unit, until, timeoutMinutes}; mode is duration/until/webhook. duration uses templated amount plus seconds/minutes/hours/days; until uses a templated ISO date; webhook pauses for a one-time callback and may set timeoutMinutes. ' +
  'Note data: {text, color}; a non-executing canvas annotation. color is yellow/blue/green/pink. Use it only to document a non-obvious section of the flow. ' +
