...
export async function GET(request: Request) {
  ...
  const flow = await prisma.flow.findFirst({ where: { id: idOf(request), organizationId: auth.organizationId } })
...
export async function DELETE(request: Request) {
  const auth = await authenticatePublicApi(request, 'flows:write')
  if (auth instanceof Response) return auth
  const result = await prisma.flow.deleteMany({ where: { id: idOf(request), organizationId: auth.organizationId } })
