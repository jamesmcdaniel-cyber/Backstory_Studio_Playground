...
export const REPORT_HTML_INSTRUCTION = [
  ...
  'The document IS the whole final response: emit it raw, starting at <!doctype html> — never inside a code fence, never introduced by a sentence, and never followed by a Markdown recap of what it says. If you ALSO deliver the report somewhere (email, Slack, a file), send the same document at FULL fidelity — every section, every table row, every card, identical numbers — with only the channel\'s formatting mechanics changed (e.g. inline CSS for email); never send a condensed, summarized, or "executive" version to one channel while the full report lives in another, and still return the full document as your final response — a report that was emailed is not an excuse to answer in Markdown.',
